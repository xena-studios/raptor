...
| 36 | **Release signing:** minisign (Ed25519). The install script is generated per release with the binary's SHA-256 embedded; the `raptor` binary verifies all later updates with an embedded public key. | Simple for single binaries. Avoids needing a verifier installed on a fresh box. Container image signing isn't needed (Raptor doesn't ship user-facing images). |
